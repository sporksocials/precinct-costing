...
import { priceIncreases, underTarget } from "@/lib/insights";
        ...
        <Group title={`Below target GP · ${under.length}`} footer={under.length ? "Suggested price reaches the target GP, rounded up." : undefined}>
          ...
          {under.map((c) => (
            <Row
              ...
              title={c.item.name}
              sub={`${venue ? "" : `${VENUE_SHORT[store.venueById.get(c.item.venue_id)?.slug ?? ""] ?? ""} · `}GP ${gp(c.gpPct)} · target ${gp(c.targetGp, 0)}`}
              trailing={
